...
public class ShortIntrosort implements ShortSort {
    private static final int INSERTION_SORT_SIZE = 64;

    private static void sort(short[] array, int from, int to, int depth) {
        int n = to - from;

        if (n < INSERTION_SORT_SIZE) {
            ShortInsertionSort.sort(array, from, to);
            return;
        }

        if (depth == 0) {
            ShortHeapsort.sort(array, from, to);
            return;
        }
    ...
    private static void sort(short[] array, int from, int to, int depth, ShortComparator comp) {
        int n = to - from;

        if (n < INSERTION_SORT_SIZE) {
            ShortInsertionSort.sort(array, from, to, comp);
            return;
        }

        if (depth == 0) {
            ShortHeapsort.sort(array, from, to, comp);
            return;
        }
    ...
    private ShortIntrosort() {}

    public static final ShortIntrosort INSTANCE = new ShortIntrosort();
